...
class GitHubPublisher extends (_publisher || _load_publisher()).HttpPublisher {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            for (const release of releases) {
                ...
                if (_this.releaseType === "draft") {
                    _this.releaseStatus = `Release with tag ${_this.tag} already exists with type "${release.prerelease ? "pre-release" : "release"}", but current release type is "${_this.releaseType}"`;
                    (0, (_builderUtil || _load_builderUtil()).warn)(_this.releaseStatus);
                    return null;
                }
                ...
                if (publishedAt != null && Date.now() - publishedAt > 2 * 3600 * 1000) {
                    ...
                    _this.releaseStatus = `Release with tag ${_this.tag} published at ${new Date(publishedAt).toString()}, more than 2 hours ago`;
                    (0, (_builderUtil || _load_builderUtil()).warn)(_this.releaseStatus);
                    return null;
                }
                return release;
            ...
            _this.releaseStatus = `Release with tag ${_this.tag} doesn't exist and not created because "publish" is not "always" and build is not on tag`;
            return null;
        })();
    }
    doUpload(fileName, arch, dataLength, requestProcessor) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (release == null) {
                (0, (_builderUtil || _load_builderUtil()).warn)(`Artifact ${fileName} is not published: ${_this2.releaseStatus}`);
                return;
            }
