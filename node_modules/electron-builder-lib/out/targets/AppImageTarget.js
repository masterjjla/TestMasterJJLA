    ...
    build(appOutDir, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (process.platform === "win32" || process.env._REMOTE_BUILD) {
                ...
                return yield remoteBuilder.build(["appimage"], appOutDir, _this.packager, _this.outDir);
            }
            ...
            const stageDir = new (_targetUtil || _load_targetUtil()).StageDir(_path.join(_this.outDir, `__appimage-${(_builderUtil || _load_builderUtil()).Arch[arch]}`));
            const appInStageDir = stageDir.getTempFile("app");
            yield stageDir.ensureEmpty();
            yield copyDirUsingHardLinks(appOutDir, appInStageDir);
            const resourceName = `appimagekit-${_this.packager.executableName}`;
            const installIcons = yield _this.copyIcons(stageDir.tempDir, resourceName);
